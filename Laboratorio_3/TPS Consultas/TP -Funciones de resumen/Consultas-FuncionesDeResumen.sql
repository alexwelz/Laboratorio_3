...
INNER JOIN Billeteras B ON B.ID_Usuario = U.ID_Usuario
INNER JOIN Movimientos M ON M.ID_Billetera = B.ID_Billetera
WHERE M.TipoMovimiento = 'D' and M.Importe > 15000 AND M.Estado =1
...
INNER JOIN Billeteras B ON B.ID_Usuario = U.ID_Usuario
INNER JOIN Movimientos M ON M.ID_Billetera = B.ID_Billetera
WHERE M.TipoMovimiento = 'D'  and NSC.ID_NivelSituacionCrediticia IS NOT NULL AND M.Estado =1
...
SELECT TOP (1) WITH TIES  U.Apellidos + ' ' + U.Nombres AS 'Usuarios', MAX(T.Importe) AS 'Importe'
FROM Usuarios U
INNER JOIN Billeteras B ON B.ID_Usuario = U.ID_Usuario
INNER JOIN Transferencias T ON T.ID_BilleteraDestino = B.ID_Billetera
WHERE T.Estado = 1
GROUP BY U.Apellidos + ' ' + U.Nombres
ORDER BY MAX(T.Importe) DESC


--13 Listar la cantidad de usuarios que hayan registrado movimientos de tipo débito.
SELECT U.Apellidos+' '+U.Nombres AS 'Usuarios',COUNT(U.ID_Usuario) AS Cantidad
FROM Usuarios U
INNER JOIN Billeteras B ON B.ID_Usuario = U.ID_Usuario
INNER JOIN Movimientos M ON M.ID_Billetera = B.ID_Billetera
WHERE  M.TipoMovimiento = 'D' AND M.Estado= 1
GROUP BY  U.Apellidos+' '+U.Nombres

--Revision
SELECT * FROM Movimientos WHERE  TipoMovimiento = 'D'
...
SELECT B.Alias, T.ID_BilleteraOrigen, COUNT(T.ID_Transferencia) AS 'Cantidad Transferencias'
FROM Billeteras B
LEFT JOIN Usuarios U ON U.ID_Usuario = B.ID_Usuario
LEFT JOIN Transferencias T ON T.ID_BilleteraOrigen = B.ID_Billetera
WHERE T.Estado = 1
GROUP BY  B.Alias,  T.ID_BilleteraOrigen
...
SELECT DISTINCT U.Apellidos+' '+U.Nombres AS 'Usuarios',B.Alias , M.ID_Billetera
FROM Movimientos M 
INNER JOIN Billeteras B ON B.ID_Billetera = M.ID_Billetera
INNER JOIN Usuarios U ON U.ID_Usuario = B.ID_Usuario
WHERE M.Importe > 40000 AND DATEPART(MONTH, M.FechaHora )='8' AND DATEPART(YEAR, M.FechaHora )='2023' AND M.Estado=1
