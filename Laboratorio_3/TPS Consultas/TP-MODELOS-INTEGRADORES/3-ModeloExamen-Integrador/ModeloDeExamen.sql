...
--las tres mejores fotografías publicadas (si las hay). Indicando el nombre del concurso, apellido y nombres del
--participante, el título de la publicación, la fecha de publicación y el puntaje promedio obtenido por esa publicación.
...
As
Begin
    Select Top 3 With Ties C.Titulo As Concurso, P.Apellidos, P.Nombres,
	F.Titulo, F.Publicacion, AVG(V.Puntaje) as PProm
    From Concursos C
...
--2)
-- Hacer un procedimiento almacenado llamado SP_Descalificar que reciba un ID de fotografía y realice la
--descalificación de la misma. También debe eliminar todas las votaciones registradas a la fotografía en cuestión. 
...
--Si ocurriese un error, mostrarlo con un mensaje aclaratorio. De lo contrario, insertar el registro teniendo en 
--cuenta que la fecha de publicación es la fecha y hora del sistema.
...
--4)
-- Al insertar una votación, verificar que el usuario que vota no lo haga más de una vez para el mismo concurso 
--ni se pueda votar a sí mismo. Tampoco puede votar una fotografía descalificada. Si ninguna validación lo impide 
--insertar el registro de lo contrario, informarlo con un mensaje de error.
Go
...
--5)
-- Hacer un listado en el que se obtenga: ID de participante, apellidos y nombres de los participantes que hayan
