...
BEGIN
    BEGIN TRY
        ...
        SELECT @DocenteLegajo = Legajo, @IDMateria = ID_Materia, @AnioIngreso = AñoIngreso FROM inserted
        ...
        BEGIN
            ROLLBACK TRANSACTION
            RAISEERROR ('El docente tiene que tener al menos 5 años de antigüedad para ser profesor de una materia', 16, 1)
        END

        IF EXISTS (
            SELECT 1 FROM PlantaDocente
            WHERE ID_Materia = @IDMateria AND Año = YEAR(GETDATE()) AND Legajo <> @DocenteLegajo AND ID_Cargo = 1
        )
        BEGIN
            ROLLBACK;
            RAISEERROR ('Ya existe otro profesor en la misma materia y año', 16, 1);
        END
